...
{
   CounterPreambleNoClone(SourceExpression_BinaryAnd, SourceExpression_Binary);
...
protected:
   ...
   virtual void doGet(ObjectVector *objects, VariableType *type, int tmpBase)
   {
      ...
      case VariableType::BT_INT_LL:
         doGetBaseILB(objects, type, tmpBase, OCODE_AND_TEMP_I);
         break;

      case VariableType::BT_UNS_L:
      case VariableType::BT_UNS_LL:
         doGetBaseILB(objects, type, tmpBase, OCODE_AND_TEMP_U);
         break;
      }
   }
